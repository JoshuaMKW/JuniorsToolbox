...
namespace librii::RARC {
    ...
    static inline std::expected<const rarcNodesHeader *, Toolbox::SerialError>
    rarcGetNodesHeader(const rarcMetaHeader *self) {
        if (self->nodes.offset >= sizeof(rarcMetaHeader));
        return (const rarcNodesHeader *)(((u8 *)self) + self->nodes.offset);
    }

    static inline std::expected<const rarcDirectoryNode *, Toolbox::SerialError>
    rarcGetDirectoryNodes(const rarcNodesHeader *self) {
        if (self->dir_nodes.offset >= sizeof(rarcNodesHeader));
        return (const rarcDirectoryNode *)(((u8 *)self) + self->dir_nodes.offset);
    }

    static inline std::expected<const rarcFSNode *, Toolbox::SerialError>
    rarcGetFSNodes(const rarcNodesHeader *self) {
        if (self->fs_nodes.offset >=
    ...
    static inline std::expected<const char *, Toolbox::SerialError>
    rarcGetFSNames(const rarcNodesHeader *self) {
        if (self->strings_offset >=
               self->fs_nodes.offset + roundUp(sizeof(rarcFSNode) * self->fs_nodes.count, 32));
        return ((const char *)self) + self->strings_offset;
    }

    static inline std::expected<const u8 *, Toolbox::SerialError> rarcGetFileData(const rarcMetaHeader *self) {
        auto *node_info = TRY(rarcGetNodesHeader(self));
        if (self->files.offset >= node_info->strings_offset + node_info->string_table_size);
        return (const u8 *)((((u8 *)self) + roundUp(self->nodes.offset + self->files.offset, 32)));
    };
    ...
    static bool rarcIsSpecialPath(const std::string &name) { return name == "." || name == ".."; }
    ...
    Result<void> LoadResourceArchiveLow(LowResourceArchive &result, rsl::byte_view data) {
        if (!SafeMemCopy(result.header, data))
            return std::unexpected("Incomplete meta header data!");

        const auto *node_info =
            TRY(rarcGetNodesHeader(reinterpret_cast<const rarcMetaHeader *>(data.data())));
        ...
        auto *fd_begin =
            TRY(rarcGetFileData(reinterpret_cast<const rarcMetaHeader *>(data.data())));
    ...
    Result<std::vector<u8>> SaveResourceArchiveLow(std::size_t mram_size, std::size_t aram_size,
                                                   std::size_t dvd_size, bool ids_synced,
                                                   const std::vector<rarcDirectoryNode> &dir_nodes,
                                                   const std::vector<rarcFSNode> &fs_nodes,
                                                   const std::string &strings_blob,
        ...
        rarcNodesHeader node_header{.id_max = fs_nodes.size(), .ids_synced = ids_synced};
        ...
        rarcMetaHeader meta_header  = {.magic =
                                           static_cast<rsl::bu32>('RARC')};  // Little endian 'RARC'
        meta_header.nodes.offset    = sizeof(rarcMetaHeader);
        meta_header.files.offset    = roundUp(node_header.strings_offset + strings_blob.size(), 32);
        ...
        meta_header.size =
            meta_header.nodes.offset + meta_header.files.offset + meta_header.files.size;
        ...
        memcpy(&result[meta_header.nodes.offset + node_header.dir_nodes.offset], dir_nodes.data(),
               sizeof(rarcDirectoryNode) * node_header.dir_nodes.count);
        memcpy(&result[meta_header.nodes.offset + node_header.fs_nodes.offset], fs_nodes.data(),
               sizeof(rarcFSNode) * node_header.fs_nodes.count);
        memcpy(&result[meta_header.nodes.offset + node_header.strings_offset], strings_blob.data(),
               strings_blob.size());
        memcpy(&result[meta_header.nodes.offset + meta_header.files.offset], file_data.data(),
               file_data.size());

        if (result.size() != meta_header.size)
            return std::unexpected("Saved data mismatched the recorded data size provided!");
    ...
    static Result<SpecialDirs> rarcCreateSpecialDirs(const ResourceArchive::Node &node,
    ...
    static Result<void> rarcInsertSpecialDirs(std::vector<ResourceArchive::Node> &nodes) {
    ...
    static void rarcSortNodesForSaveRecursive(const std::vector<ResourceArchive::Node> &src,
    ...
    rarcProcessNodesForSave(const ResourceArchive &arc, children_info_type &children_info) {
        std::vector<ResourceArchive::Node> out;
    ...
    static void rarcRecurseLoadDirectory(ResourceArchive &arc, LowResourceArchive &low,
        ...
        auto *fs_end   = low.fs_nodes.data() + (low_dir.children_offset + low_dir.child_count);
    ...
    bool IsDataResourceArchive(rsl::byte_view data) {
        return data.size() >= 4 && data[0] == 'R' && data[1] == 'A' && data[2] == 'R' &&
               data[3] == 'C';
    }
    ...
    Result<ResourceArchive> LoadResourceArchive(rsl::byte_view data) {
        ...
        rarcRecurseLoadDirectory(result, low, low.dir_nodes[0], std::nullopt, std::nullopt,
                                 result.nodes, 0);
        return result;
    }
    ...
    Result<std::vector<u8>> SaveResourceArchive(const ResourceArchive &arc, bool make_matching,
        ...
        for (auto &node : arc.nodes) {
            OffsetInfo offsets =
                offsets_map[make_matching ? node.name + std::to_string(node.id) : node.name];
        ...
        for (auto &node : processed_nodes) {
            ...
            OffsetInfo offsets =
                offsets_map[make_matching && !is_special_dir ? node.name + std::to_string(node.id)
                                                             : node.name];
        ...
        return SaveResourceArchiveLow(mram_size, aram_size, dvd_size, ids_synced, dir_nodes,
                                      fs_nodes, strings_blob, low_data);
    }
    ...
    Result<void> ExtractResourceArchive(const ResourceArchive &arc, std::filesystem::path out) {
        ...
        for (auto &node : arc.nodes) {
            ...
            if (node.is_folder()) {
                ...
                fout.write(reinterpret_cast<const char *>(node.data.data()), node.data.size());
            }
        }
        return {};
    }
    ...
    Result<ResourceArchive> CreateResourceArchive(std::filesystem::path root) {
        ...
        for (auto &path : sorted_fs_tree) {
            ...
            path = std::filesystem::path(".") / FS_TRY(rsl::filesystem::relative(path, root));
    ...
    Result<void> ImportFiles(ResourceArchive &rarc, ResourceArchive::Node parent,
        ...
        int parent_index = std::distance(rarc.nodes.begin(),
    ...
    bool DeleteNodes(ResourceArchive &rarc, std::vector<ResourceArchive::Node> &nodes) {
        if (nodes.size() == 0)
            return false;
            ...
            for (int i = 0; i < rarc.nodes.size(); i++) {
                if (to_delete == rarc.nodes[i]) {
                    if (!to_delete.is_folder()) {
                        ...
                        auto deleted_at = std::distance(rarc.nodes.begin(),
    ...
    Result<void> ExtractNodeTo(const ResourceArchive &rarc, ResourceArchive::Node node,
        ...
        if (node.is_folder()) {
            ...
            tmp_rarc.nodes[0].folder = {-1,
                                        tmp_rarc.nodes[0].folder.sibling_next - (s32)before_size};

            tmp_rarc.nodes[1].folder = {-1, tmp_rarc.nodes[0].folder.sibling_next};

            tmp_rarc.nodes[2].folder = {-1, 0};
            for (auto tnode = tmp_rarc.nodes.begin() + 3; tnode != tmp_rarc.nodes.end(); tnode++) {
                if (tnode->is_folder()) {
    ...
    Result<void> ReplaceNode(ResourceArchive &rarc, ResourceArchive::Node to_replace,
        ...
        if (to_replace.is_folder()) {
            ...
            auto parent =
                std::find_if(rarc.nodes.begin(), rarc.nodes.end(), [&](ResourceArchive::Node node) {
