...
namespace Toolbox::UI {
...
#if 1
        if (ImGui::InputComboTextBox("##AddressComboBox", m_address_input.data(),
                                     m_address_input.size(), history_strs,
                                     m_address_search_history.size(), nullptr, ImGuiComboFlags_None,
            ...
            if (input_str.length() == 8 && std::all_of(input_str.begin(), input_str.end(),
...
#else
        if (ImGui::BeginCombo("##AddressComboBox", m_base_address, ImGuiComboFlags_NoPreview)) {
            for (u32 addr : m_address_search_history) {
                bool selected = (addr == m_base_address);

                std::string addr_str = std::format("0x{:08X}", addr);
                if (ImGui::Selectable(addr_str.c_str(), &selected)) {
                    m_base_address = addr;
                }
            }
    ...
    static u32 renderMemoryRow(void *handle, u32 base_address, u32 byte_limit, u8 column_count,
        ...
        byte_width              = std::min<u8>(byte_width, 8);
    ...
    void DebuggerWindow::renderMemoryView() {
        if (ImGui::BeginChild("Memory (MRAM)", {0, 0}, true,
            ...
            u32 row_count = (u32)(size.y / ImGui::GetTextLineHeightWithSpacing() +
                                  0.5f);
            if (row_count == 0) {
                row_count = 1;
            }
            ...
            float column_count_pred =
                std::max<float>(size.x - address_col_width, 0.0f) / full_col_width + 0.5f;
            u32 column_count = (column_count_pred * 0.76f);  // Give space for ascii column
