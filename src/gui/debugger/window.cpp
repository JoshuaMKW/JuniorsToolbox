...
namespace Toolbox::UI {
    ...
    DebuggerWindow::DebuggerWindow(const std::string &name) : ImWindow(name) {}
    ...
    static u32 renderMemoryRow(void *handle, u32 base_address, u8 column_count, u8 byte_width) {
        // Render the address
        // ---
        ImGui::Text("0x%08X", base_address);

        byte_width        = std::min<u8>(byte_width, 8);
        char formatter[8] = {'%', '0', '0' + byte_width, 'X', '\0'};
        ...
        for (u8 column = 0; column < column_count; ++column) {
            ...
            // ...
            // ---
            u64 mask = ~(0xFFFFFFFF'FFFFFFFF >> (byte_width * 8));
            ImGui::Text(formatter, mask);
            
            handle = (u8 *)handle + byte_width;
        }

        return column_count * byte_width;
    }
    ...
    void DebuggerWindow::renderMemoryView() {
        if (ImGui::BeginChild("Memory (MRAM)", {0, 0}, true,
            ...
            u32 row_count = (u32)(size.y / (style.FramePadding.y * 2.0f +
            ...
            u32 column_count = (u32)(size.x / (style.FramePadding.x * 2.0f +
                                               ImGui::GetTextLineHeightWithSpacing()) +
                                     0.5f);
            if (column_count == 0) {
