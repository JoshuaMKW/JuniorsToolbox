...
namespace Toolbox::UI {
    ...
    */
    ...
    bool SceneWindow::postUpdate(f32 delta_time) {
        if (!m_is_render_window_focused || !m_renderer.isGizmoManipulated()) {
            return true;
        }
    ...
    void SceneWindow::renderBody(f32 delta_time) {
        renderHierarchy();
        renderProperties();
        renderRailEditor();
        renderScene(delta_time);
        processSelection(delta_time);
    }
        ...
        m_rail_list_multi_node_menu.addOption(
            "Delete", {GLFW_KEY_DELETE}, [this](std::vector<SelectionNodeInfo<Rail::Rail>> info) {
                ...
                for (auto &select : info) {
                    ...
                    data.removeRail(*select.m_selected);
                }
                m_current_scene->setRailData(data);
                m_update_render_objs = true;
                return;
    ...
    void SceneWindow::processSelection(f32 delta_time) {
