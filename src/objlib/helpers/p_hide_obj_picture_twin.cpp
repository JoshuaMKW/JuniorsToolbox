...
static void HelperSetHideObjPictureTwinMaterialColors(Toolbox::RefPtr<J3DModelData> model_data,
                                                      Toolbox::Color::RGB32 color) {
...
void PhysicalSceneObject::HelperUpdateHideObjPictureTwinRender() {
    RefPtr<Object::MetaMember> color_member = getMember("Color").value_or(nullptr);
    if (!color_member) {
        TOOLBOX_DEBUG_LOG("Failed to get parameter for HideObjPictureTwin!");
    } else {
        Color::RGB32 color = getMetaValue<Color::RGB32>(color_member, 0).value();
        HelperSetHideObjPictureTwinMaterialColors(m_model_data, color);
    }
