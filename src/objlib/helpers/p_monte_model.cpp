...
static void HelperSetMonteMaterialColors(Toolbox::RefPtr<J3DModelData> model_data,
    ...
    std::shared_ptr<J3DMaterial> mat_fuku     = model_data->GetMaterial("_fuku_mat");
    std::shared_ptr<J3DMaterial> mat_body     = model_data->GetMaterial("_hand_mat");
    std::shared_ptr<J3DMaterial> mat_leafback = model_data->GetMaterial("_leafback_mat");
    ...
    if (mat_leafback) {
        mat_leafback->TevBlock->mTevKonstColors[0].a = weighted_pol_strength;
    }
}
