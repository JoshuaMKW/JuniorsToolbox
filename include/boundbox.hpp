...
namespace Toolbox {
    ...
    struct BoundingBox {
        ...
        glm::vec3 sample(f32 lx, f32 ly, f32 lz, f32 scale = 1.0f,
            ...
            glm::mat4x4 rot = glm::identity<glm::mat3x3>();
            rot *= m_rotation;
        ...
        bool contains(const glm::vec3 &point, f32 scale = 1.0f,
            ...
            if (type == BoundingType::Box) {
                return (fabsf(inv_point.x) <= m_size.x * 0.5f * scale) &&
                       (fabsf(inv_point.y) <= m_size.y * 0.5f * scale) &&
                       (fabsf(inv_point.z) <= m_size.z * 0.5f * scale);
            } else {
